                ...
                <div>
                    <div className={styles.imageBtnContainer}>
                        ...
                        <div className={styles.imgLowerContainer}>
                            <div className={ moduleType ? styles.imgFileLabelPlus1 : styles.imgFileLabelPlus}>
                                <span className={styles.plus}>{!value && !default_image ? '+' :''}</span>
                                {/* {partnerImage ? <div> "Partner image" </div  */}
                                <div className={styles.textUpload} style={error ? {} : {}}>{!value  && !default_image ? (moduleType ? 'Upload Image' : "Upload"):''}</div></div>
        ...
        if(bannerLabel && !multiple ){
            return (
                ...
                <div className={bannerLabelTrue === 'true' ? styles.imageBanner :  styles.imageBtnContainerShow}>
                    <div>
                        <div className={bannerimg === "true" ? styles.bannerImageSize : csx(styles.imagePlusShow, this.props.imageClass)} style={{ backgroundImage: "url("+(this._getImageUrl(value))+")",
                            backgroundSize: 'cover', backgroundPosition: 'center',borderColor: (error ? 'red' : '#c2c2c2')}}></div>
                    </div>
                    <div className={styles.imgLowerContainer}>
                        <div className={ bannerLabelTrue === 'true' ? styles.imgFileLabel12 : bannerimg === "true" ? styles.imgFileLabelPlusShowSIZE : styles.imgFileLabelPlusShow}>
                            <span className={styles.plus}>{!value && !default_image ? '+' :''}</span>

...
import { serviceCalendarCountList } from "../services/CalendarCount.service";
...
export function actionFetchCalendarCount(index = 1,event_id={event_id},
    sorting = {}, filter = {}) {
    const request = serviceCalendarCountList({
        index,
        event_id,
        row: sorting.row,
        order: sorting.order,
...
export function actionCreateCalendarCount(data) {
...
export function actionUpdateCalendarCount(data) {
...
export function actionDeleteCalendarCount(id) {
...
export function actionChangePageCalendarCount(page) {
    return (dispatch) => {
        dispatch({ type: CHANGE_PAGE, payload: page });
    };
}

export function actionFilterCalendarCount(value) {
...
export function actionChangeStatusCalendarCount(id, status) {
    return (dispatch) => {
        dispatch({ type: CHANGE_STATUS, payload: { id, status } });
    };
}

export function actionResetFilterCalendarCount() {
    return {
        type: RESET_FILTER,
        payload: null,
    };
...
export function actionSetPageCalendarCount(page) {
    const stateData = store.getState().calendar_reducer;
    ...
    if (totalLength <= (page + 1) * Constants.DEFAULT_PAGE_VALUE) {
        store.dispatch(
            actionFetchCalendarCount(serverPage + 1, sortingData, {
                query,

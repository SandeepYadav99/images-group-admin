...
import {
  ...
  serviceGetKnowledgeStampList,
  serviceUpdateKnowledgeStampList,
  serviceDeleteKnowledgeStampList,
...
export function actionFetchKnowledgeList(index = 1, sorting = {}, filter = {}) {
  const request = serviceGetKnowledgeStampList({
    index,
    row: sorting.row,
    order: sorting.order,
    ...filter,
...
export function actionCreateKnowledgeList(data) {
  const request = serviceCreateKnowledgeStampList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionUpdateKnowledgeList(data) {
  const request = serviceUpdateKnowledgeStampList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionDeleteKnowledgeList(id) {
  const request = serviceDeleteKnowledgeStampList({ id: id });
  return (dispatch) => {

...
import {
  serviceCreateMeetingRoomSlotList,
  serviceGetMeetingRoomSlotList,
  serviceUpdateMeetingRoomSlotList,
  serviceDeleteMeetingRoomSlotList,
} from "../services/MeetingSlots.service";
...
export function actionCreateMeetingRoomSlotList(data) {
  const request = serviceCreateMeetingRoomSlotList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionUpdateMeetingRoomSlotList(data) {
  const request = serviceUpdateMeetingRoomSlotList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionDeleteMeetingRoomSlotList(id) {
  const request = serviceDeleteMeetingRoomSlotList({ id: id });
  return (dispatch) => {
    dispatch({ type: DELETE_ITEM, payload: id });
  };
}

export function actionChangePageMeetingRoomSlotList(page) {
  return (dispatch) => {
    dispatch({ type: CHANGE_PAGE, payload: page });
  };
}

export function actionFilterMeetingRoomSlotList(value) {
...
export function actionChangeStatusMeetingRoomSlotList(id, status) {
  return (dispatch) => {
    dispatch({ type: CHANGE_STATUS, payload: { id, status } });
  };
}

export function actionResetFilterMeetingRoomSlotList() {
  return {
    type: RESET_FILTER,
    payload: null,
  };
...
export function actionSetPageMeetingRoomSlotList(page) {
  const stateData = store.getState().meeting_slots;
  ...
  if (totalLength <= (page + 1) * Constants.DEFAULT_PAGE_VALUE) {
    store.dispatch(
        actionFetchMeetingRoomSlotList(serverPage + 1, sortingData, {
        query,

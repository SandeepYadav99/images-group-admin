...
import { serviceCreateCustomParticipant, serviceGetCustomParticipant, serviceUpdateCustomParticipant } from "../services/CustomParticipant.service";
...
export function actionFetchCustomParticipant(
  index = 1,
  sorting = {},
  filter = {},
  shouldReset = false
) {
  const request = serviceGetCustomParticipant({
    index,
    row: sorting.row,
    order: sorting.order,
    ...filter,
...
export function actionCreatePolicyList(data) {
  const request = serviceCreateCustomParticipant(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionUpdatePolicyList(data) {
  const request = serviceUpdateCustomParticipant(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionFilterCustomParticipant(value) {
...
export function actionChangeStatusCustomParticipant(id, status) {
...
export function actionResetFilterCustomParticipant() {
  return {
    type: RESET_FILTER,
    payload: null,
  };
...
export function actionSetPageCustomParticipant(page) {
  const stateData = store.getState().custom_participant;
  ...
  if (totalLength <= (page + 1) * Constants.DEFAULT_PAGE_VALUE) {
    store.dispatch(
      actionFetchCustomParticipant(serverPage + 1, sortingData, {
        query,

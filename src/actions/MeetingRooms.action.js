...
import {
  ...
  serviceGetMeetingRoomList,
  serviceUpdateMeetingRoomList,
  serviceDeleteMeetingRoomList,
...
export function actionFetchMeetingRoomList(index = 1,event_id ,sorting = {}, filter = {}) {
  const request = serviceGetMeetingRoomList({
    index,
    event_id,
    row: sorting.row,
    order: sorting.order,
...
export function actionCreateMeetingRoomList(data) {
  const request = serviceCreateMeetingRoomList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionUpdateMeetingRoomList(data) {
  const request = serviceUpdateMeetingRoomList(data);
  return (dispatch) => {
    request.then((data) => {
      if (!data.error) {
...
export function actionDeleteMeetingRoomList(id) {
  const request = serviceDeleteMeetingRoomList({ id: id });
  return (dispatch) => {
    dispatch({ type: DELETE_ITEM, payload: id });
  };
}

export function actionChangePageMeetingRoomList(page) {
  return (dispatch) => {
    dispatch({ type: CHANGE_PAGE, payload: page });
  };
}

export function actionFilterMeetingRoomList(value) {
...
export function actionChangeStatusMeetingRoomList(id, status) {
  return (dispatch) => {
    dispatch({ type: CHANGE_STATUS, payload: { id, status } });
  };
}

export function actionResetFilterMeetingRoomList() {
  return {
    type: RESET_FILTER,
    payload: null,
  };
...
export function actionSetPageMeetingRoomList(page) {
  ...
  if (totalLength <= (page + 1) * Constants.DEFAULT_PAGE_VALUE) {
    store.dispatch(
        actionFetchMeetingRoomList(serverPage + 1, sortingData, {
        query,
